...
for c in range(maxIter):
    ...
    with w.if_todo('c%02i_merge_h5' % c):
        ...
        if c > 1:
            s.add(f'h5_merger.py --h5_out {sol_dir}/cal-amp-dd-merged-c{c} --h5_tables {sol_dir}/cal-amp-dd-c{c} --h5_time_freq {sol_dir}/cal-tec2-c{c}.h5 \
              --no_antenna_crash --no_pol' , log='h5_merger.log', commandType='python')
            s.run(check=True)
            ...
            s.add(
                f'h5_merger.py --h5_out {sol_dir}/cal-tec-merged-c{c}.h5 --h5_tables {sol_dir}/cal-tec-RS-c{c}.h5 {sol_dir}/cal-tec-CS-c{c}.h5 \
                   --h5_time_freq {sol_dir}/cal-tec2-c{c}.h5 --no_antenna_crash --no_pol', log='h5_merger.log',
                commandType='python')
            s.run(check=True)
            lib_util.run_losoto(s, f'tec-merged-c{c}', f'{sol_dir}/cal-tec-merged-c{c}.h5',
